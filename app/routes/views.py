...
@views_bp.route('/dini-gunler')
@cache.cached(timeout=3600)
def dini_gunler():
    og_image_url = url_for(
        'og.og_image',
        title     = 'Dini Günler ve Kandiller',
        subtitle  = 'Ramazan, bayramlar ve diğer önemli İslami günler hakkında bilgi.',
        theme     = 'religious-days',
        icon      = r'\udb82\udd79',
        prompt    = 'Dini Günler',
        domain    = 'cagrivakti.com.tr',
        _external = True,
...
def status():
    return redirect('https://status.cagrivakti.com.tr', code=301)

@views_bp.route('/durum')
def durum():
    return redirect('https://durum.cagrivakti.com.tr', code=301)
