...
# ════════════════════════════════════════════
#  STATIC — favicon, sw.js, manifest, sitemap, robots
# ════════════════════════════════════════════
            ...
            for shortcut in manifest_data["shortcuts"]:
                if shortcut.get("url") == "/konum-bul":
                    ...
                    if user_city:
                        ...
                        shortcut["icons"] = [{"src": "/static/icons/android/android-launchericon-96-96.png", "sizes": "96x96", "type": "image/png"}]
                    break
            if not found and user_city:
                display_name = CITY_DISPLAY_NAME_MAPPING.get(user_city, user_city)
                manifest_data["shortcuts"].insert(0, {
                    "name": f"{display_name} Vakitleri",
                    "short_name": display_name,
                    "url": f"/sehir/{user_city}",
                    "icons": [{"src": "/static/icons/android/android-launchericon-96-96.png", "sizes": "96x96", "type": "image/png"}]
                })
...
def serve_sitemap():
    ...
    for rule in static_urls:
        pages.append({"loc": url_for(rule, _external=True),
                      "lastmod": datetime.now().strftime("%Y-%m-%d"),
                      "priority": "1.0" if rule == 'views.index' else "0.8"})
    for sehir in UserService.get_sehirler('ALL'):
        country = get_country_for_city(sehir)
        pages.append({"loc": url_for('views.sehir_sayfasi', sehir=sehir, _external=True),
                      "lastmod": datetime.now().strftime("%Y-%m-%d"), "priority": "0.9"})
        if country == 'TR':
            pages.append({"loc": url_for('views.imsakiye_detay', sehir=sehir, _external=True),
...
def serve_robots():
    content = ("User-agent: *\n"
               ...
               f"Sitemap: {url_for('views.serve_sitemap', _external=True)}\n")
...
def sehir_sayfasi(sehir):
    ...
    country_code = request.args.get('country') or get_country_for_city(sehir) or 'TR'
    ...
    response = make_response(render_template('city/city_page.html',
                         sehir=sehir, country_code=country_code, vakitler=vakitler,
                         daily_content=get_daily_content(),
                         ramadan_info=RamadanService.get_ramadan_info(),
...
def sehir_secimi():
    ...
    city_data = sorted([{'name': c, 'country': get_country_for_city(c)} for c in all_cities],
                       key=lambda x: x['name'])
    return render_template('city/city_selection.html', cities=city_data)


# ════════════════════════════════════════════
#  RAMADAN — Ramazan & imsakiye
# ════════════════════════════════════════════
...
def imsakiye_detay(sehir):
    ...
    return render_template('ramadan/ramadan_schedule_detail.html',
                         sehir=sehir, country_code=country_code,
                         ramadan_info=RamadanService.get_ramadan_info())
...
def bilgi_kosesi_liste():
    return render_template('guide/guide_list.html', guides=get_guides())
...
def bilgi_kosesi_detay(slug):
    ...
    if not guide:
        abort(404)
    return render_template('guide/guide_detail.html', guide=guide, guides=get_guides())
...
def sitene_ekle():
    ...
    return render_template('embed/builder.html', cities=all_cities,
                         seo_title=title, seo_description=description)
...
def embed_widget(sehir):
    ...
    etag = hashlib.md5(f"{widget_version}-{sehir}-{current_date}-{theme}-{bg_color}-{text_color}".encode()).hexdigest()
    if request.if_none_match and request.if_none_match.contains(etag):
        return "", 304
    response = make_response(render_template('embed/widget.html',
                         sehir=sehir, display_name=display_name, vakitler=vakitler,
                         theme=theme, bg_color=bg_color, text_color=text_color))
...
def admin_content_edit(content_id=None):
    ...
    if request.method == 'POST':
        ...
        content.is_active = 'is_active' in request.form
        try:
            db.session.commit()
...
def admin_logs():
        ...
        try:
            ...
            for line in lines:
                ...
                if match:
                    ...
                    try:
                        hour = (timestamp_str.split(' ')[1][:2] + ":00") if ' ' in timestamp_str else (timestamp_str[11:13] + ":00" if len(timestamp_str) > 13 else "00:00")
    ...
    for log_path, attr in [(api_log_file, 'api'), (bot_log_file, 'bot'), (security_log_file, 'security')]:
        if log_path and os.path.exists(log_path):
            try:
                with open(log_path, 'r', encoding='utf-8') as f:
                    val = "".join(f.readlines()[-200:])
                if attr == 'api': api_logs = val
                elif attr == 'bot': bot_logs = val
                elif attr == 'security': security_logs = val
            except Exception as e:
                current_app.logger.error(f"Log okuma hatası ({attr}): {e}")
    ...
    return render_template('admin/logs.html',
                         web_logs=web_logs, api_logs=api_logs,
                         bot_logs=bot_logs, security_logs=security_logs,
                         stats=stats)


# ════════════════════════════════════════════
#  EXTRA — Oyun, QR, asal sayı, rainmeter
# ════════════════════════════════════════════
