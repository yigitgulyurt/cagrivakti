function updateOnlineStatus() {
    ...
    // Kullanıcının bahsettiği "kırmızı kutu" uyarısı burada tetiklenmi
    console.log(navigator.onLine ? "Çevrimiçi" : "Çevrimdışı");
}
window.addEventListener('online', updateOnlineStatus);
window.addEventListener('offline', updateOnlineStatus);
// Manifest URL'ini kullanıcı şehrine göre güncelle (Cache busting ve P
window.addEventListener('DOMContentLoaded', () => {
    ...
    if (userCityMatch && userCityMatch[1]) {
        ...
        if (manifestLink) {
            const url = new URL(manifestLink.href, window.location.orig
...
// Şehir eşleştirme ve normalizasyon fonksiyonları (Global Scope'da tan
const CITY_DISPLAY_MAPPING_GLOBAL = JSON.parse('{{ CITY_DISPLAY_NAME_MA
const REVERSE_CITY_MAPPING_GLOBAL = Object.fromEntries(
    Object.entries(CITY_DISPLAY_MAPPING_GLOBAL).map(([latin, turkish]) 
);
function getLatinNameGlobal(turkishName) {
     ...
     let normalized = turkishName.split('').map(char => charMap[char] |
...
window.addEventListener('beforeinstallprompt', (e) => {
    ...
    // İndir sayfasında ise native banner'ın çıkmasına izin ver (preven
    // Diğer sayfalarda otomatik gösterimi engelle
    const isDownloadPage = window.location.pathname.includes('/indir') 
    ...
    if (sessionStorage.getItem('reloadingForPWA')) {
        ...
        if (pwaText) pwaText.textContent = 'Her şey hazır, şimdi uygula
        if (installBtn) {
            installBtn.innerHTML = '<svg class="icon" style="font-size:
            installBtn.classList.add('pulse-animation');
        }
        
        sessionStorage.removeItem('reloadingForPWA');
    } 
    // Eğer kullanıcı daha önce bu oturumda kapatmadıysa banner'ı göste
...
window.addEventListener('load', () => {
     const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobi
     ...
     if (isMobile && !isStandalone && !isDismissed && !isDownloadPage) 
         // beforeinstallprompt'un tetiklenip tetiklenmediğini kontrol 
         setTimeout(() => {
             if (!window.deferredPrompt) {
                 // Eğer native prompt yoksa, banner'ı yine de göster (
                 if (pwaBanner) {
                     pwaBanner.classList.add('active');
                     // Buton metnini gerekirse güncelle (Şimdilik "Yük
                 }
             }
         }, 3000); // 3 saniye bekle
     }
...
if (installBtn) {
    installBtn.addEventListener('click', async () => {
        if (window.deferredPrompt) {
            ...
            const userCity = document.cookie.split('; ').find(row => ro
            const pwaText = document.querySelector('.pwa-text p');
            
            if (!userCity) {
                // MODAL VE TOAST ELEMENTLERİNİ SEÇ
                const permissionModal = document.getElementById('locati
                const allowBtn = document.getElementById('allowLocation
                const denyBtn = document.getElementById('denyLocationBt
                ...
                if (permissionModal) {
                    ...
                    const handleAllow = async () => {
                        ...
                        // Toast göster
                        if (toast && toastMsg) {
                            toastMsg.textContent = 'Konum uydudan alını
                            toast.classList.add('active');
                            toast.classList.remove('success'); // Reset
                        }
                        try {
                            // Konum izni iste
                            ...
                            // Konum alındı
                            if (toastMsg) toastMsg.textContent = 'Şehir
                            ...
                            try {
                                const response = await fetch(`https://n
                                if (response.ok) {
                                    const data = await response.json();
                                    if (data && data.address) {
                                        originalCityName = data.address
                                    }
                                }
                            } catch (err) {
                                console.warn('Nominatim failed, trying 
                            }
                            ...
                            if (!originalCityName) {
                                try {
                                    ...
                                    if (response.ok) {
                                        ...
                                        originalCityName = data.city ||
                                    }
                                } catch (err) {
                                    console.error('Fallback geocoding f
                                }
                            }
                            if (originalCityName) {
                                // Başarılı
                                originalCityName = originalCityName.rep
                                const latinCityName = getLatinNameGloba
                                
                                // Cookie ve LocalStorage güncelle
                                document.cookie = `user_city=${latinCit
                                localStorage.setItem('user_city', latin
                                
                                if (toastMsg) toastMsg.textContent = `Ş
                                if (toast) {
                                    toast.classList.add('success');
                                    // Başarı ikonunu göster (spinner g
                                    const icon = toast.querySelector('.
                                    if(icon) icon.setAttribute('xlink:h
                                }
                                
                                // Biraz bekle sonra yenile
                                setTimeout(() => {
                                    sessionStorage.setItem('reloadingFo
                                    window.location.reload();
                                }, 1500);
                            } else {
                                // Başarısız - Manuel Seçime Yönlendir
                                console.error('All reverse geocoding at
                                if (toastMsg) toastMsg.textContent = 'O
                                if (toast) toast.classList.remove('succ
                            ...
                            if (toastMsg) toastMsg.textContent = 'Konum
                            setTimeout(() => {
                                if (toast) toast.classList.remove('acti
                            }, 2000);
                        }
                        
                        // Event listener'ı temizle (gerekirse)
                        allowBtn.removeEventListener('click', handleAll
                    };
                    allowBtn.onclick = handleAllow; // onclick kullanar
            ...
            // Fallback: Native prompt yoksa (iOS vb.) indir sayfasına 
...
if (navToggle && navLinks) {
    ...
    navLinks.querySelectorAll('.nav-link').forEach(link => {
        link.addEventListener('click', () => {
            ...
            navToggle.querySelector('use').setAttribute('xlink:href', '
            document.body.style.overflow = 'auto';
        });
    });
}